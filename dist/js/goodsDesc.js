define(['jquery',"jquery-cookie"],function($){
    ...
    function banner(){
        ...
        //事件委托  实现点击按钮完成切换
        $("#app div").on("click",".ui-pager .ui-pager-item",function(){
        ...
        function tab(){
            if(!aImage){
                aImage=$("#J_img").find("img");
            }
            if(!aBtns){
                aBtns=$("#J_img").find(".ui-pager .ui-pager-item a");
            }
            if(aImage.size()==1){
                clearInterval(timer);
            }else{
                ...
                aImage.hide().eq(iNow).show();
            }
        }

        //给加入购物车按钮，添加点击操作
        $("#app div").on("click",".J_login",function(){
            //获取当前点击“加入购物车按钮”的商品id
            //cookie本地缓存（最大4kb，只能字符串）,  存商品id，和数量就可以了
            //[{id:1,num},{id:2,num}]转成json格式字符串存储在cookie中
            var id=this.id;
            //1,判断是不是第一次添加
